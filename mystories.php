...
if (isset($_GET['search'])) {
    ...
    $stmt = $pdo->prepare('SELECT posts.*, users.username, users.image AS user_image FROM posts JOIN users ON posts.user_id = users.id WHERE posts.content LIKE ? AND posts.user_id = ? ORDER BY created_at DESC');
    $stmt->execute(['%' . $searchQuery . '%', $_SESSION['user_id']]);
    $posts = $stmt->fetchAll();
} else {
    $posts = $pdo->query('SELECT posts.*, users.username, users.image AS user_image FROM posts JOIN users ON posts.user_id = users.id WHERE posts.user_id = '.$_SESSION['user_id'].' ORDER BY created_at DESC')->fetchAll();
}

//count stories
$ststmt = $pdo->prepare('SELECT COUNT(*) AS allstoriescount FROM posts WHERE user_id = ?');
$ststmt->execute([$_SESSION['user_id']]);
$allstoriescount = $ststmt->fetchColumn();
...
<head>
    ...
    <title>Daily Stories - Stories</title>
            ...
            <div class="col-md-4">
                <div class="userprofilecard sticky-top p-5">
                    ...
                    <div class="userprofileitems">
                        ...
                        <div class="upitem active"><a href="mystories.php">Stories &nbsp;&nbsp;<span><?php echo $allstoriescount; ?></span></a></div>
